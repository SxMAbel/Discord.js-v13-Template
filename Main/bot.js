...
readdirSync("./Main/commands/").forEach((dir) => {
  const commandFiles = readdirSync(`./Main/commands/${dir}/`).filter((f) =>
    f.endsWith(".js")
  );
...
readdirSync("./Main/slashCommands/").forEach((dir) => {
  const slashCommandFile = readdirSync(`./Main/slashCommands/${dir}/`).filter(
...
client.on("ready", async () => {
  await client.application.commands
    .set(data)
    .then(() => console.log(`[ Slash Command ] Application (/) Registered.`))
